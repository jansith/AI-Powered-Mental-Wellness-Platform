...
class LoginView(APIView):
    ...
    def post(self, request):
        ...
        if serializer.is_valid():
            ...
            if user.role == "doctor":
                # Check if doctor has a profile and get status from there
                if hasattr(user, 'doctor_profile'):
                    response_data['profile_status'] = user.doctor_profile.status
                    # You can also add doctor-specific fields here if needed
                else:
                    response_data['profile_status'] = 'no_profile'
                    
            elif user.role == "patient":
                if hasattr(user, 'patient_profile'):
                    response_data['patient_id'] = user.patient_profile.patient_id
                else:
                    response_data['patient_id'] = 'no_profile'
